...
export function AdminSidebar({
  ...
  return (
    <aside className="hidden w-60 shrink-0 border-r-2 border-line bg-white lg:block">
      <div className="sticky top-0 flex h-screen flex-col p-5">
        <div className="mb-6">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted">
            Panel Admin
          </p>
          <p className="font-display text-lg font-extrabold">{brandName}</p>
        </div>
            ...
            return (
              <Link
                ...
                className={`rounded-xl px-3.5 py-2.5 text-sm font-semibold transition-colors ${
                  active
                    ? "bg-tosca text-white"
                    : "text-muted hover:bg-tosca-soft hover:text-tosca-dark"
                }`}
              >
                {link.label}
              </Link>
        ...
        <div className="mt-auto pt-4">
          <Link href="/" target="_blank" className="text-xs font-semibold text-tosca-dark hover:underline">
            ↗ Lihat situs publik
          </Link>
          {!authEnabled && (
            <p className="mt-3 rounded-xl border-2 border-orange-soft bg-orange-soft/60 px-3 py-2 text-[11px] font-medium text-orange-dark">
        ...
        return (
          <Link
            ...
            className={`shrink-0 rounded-full px-3.5 py-2 text-xs font-bold transition-colors ${
              active ? "bg-tosca text-white" : "bg-cream text-muted"
            }`}
          >
            {link.label}
          </Link>
